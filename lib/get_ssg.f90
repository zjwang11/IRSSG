...
contains
  ...
  ! and stores them in the provided arrays.
    ...
    if (isSpinor) then
      open(unit=ssg_file_unit, file='msg.data', form='unformatted', access='stream', status='old', action='read', iostat=ios)
    else
  ...
  subroutine output_ssg_operator(ssg_label,nsym,rotation,tau,spin_SU2,spin_rotation,time_reversal)
    ...
    write(tmp,'(I0)')nsym
    if (isSpinor) then
    write(*,'(A)')'the list of '//trim(adjustl(tmp))//' MSG operators:    { Ui ||  Ri  |  taui }'
    else
